...
$page_title = "Editar Impresora - Sistema de Inventario";
require_once 'includes/header.php';

// Redirige si el usuario no está logueado
if (!isset($_SESSION['user_id'])) {
    $_SESSION['flash_error'] = "Debes iniciar sesión para editar impresoras.";
...
// 1. Cargar datos de la impresora existente si el ID es válido
if ($impresora_id) {
    try {
        $stmt = $pdo->prepare("SELECT * FROM impresoras WHERE id = ?");
        $stmt->bindParam(1, $impresora_id, PDO::PARAM_INT);
        $stmt->execute();
        $impresora = $stmt->fetch(PDO::FETCH_ASSOC);

        if (!$impresora) {
            $_SESSION['flash_error'] = "ID de impresora no especificado o inválido.";
            header("Location: impresoras.php");
            exit();
        }
        // Precargar form_data con los datos existentes para el formulario
        $form_data = $impresora;

    } catch (PDOException $e) {
        error_log("Error al cargar datos de impresora para edición: " . $e->getMessage());
        $_SESSION['flash_error'] = "Error interno al cargar la impresora.";
        header("Location: impresoras.php");
        exit();
    }
} else {
...
// 2. Procesar el formulario cuando se envía (actualización)
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $url_interfaz = filter_input(INPUT_POST, 'url_interfaz', FILTER_VALIDATE_URL);
    ...
    $fecha_adquisicion = filter_input(INPUT_POST, 'fecha_adquisicion', FILTER_SANITIZE_STRING);
    ...
    // Actualizar form_data con los datos enviados para repoblar el formulario en caso de error
    $form_data = [
    ...
    if (empty($errors)) {
        try {
            $sql = "UPDATE impresoras SET
                        ...
                        url_interfaz = :url_interfaz,
                        ubicacion = :ubicacion,
                        estado = :estado,
                        consumible_tipo = :consumible_tipo,
            ...
            $stmt->bindParam(':id', $impresora_id, PDO::PARAM_INT); // ID para el WHERE

            $stmt->execute();

            $_SESSION['flash_message'] = "Impresora '" . htmlspecialchars($nombre) . "' actualizada exitosamente.";
            header("Location: impresoras.php"); // Redirige a la lista de impresoras
            exit();

        } catch (PDOException $e) {
            error_log("Error al actualizar impresora: " . $e->getMessage());
            $errors[] = "Error al actualizar la impresora: " . $e->getMessage();
            $_SESSION['flash_error'] = "Error de base de datos al actualizar la impresora. Por favor, inténtelo de nuevo.";
        }
    } else {
...
// Usamos los datos cargados ($impresora) si no se ha enviado el formulario,
// o los datos de form_data si hubo errores de validación.
...
<h1 class="page-title">Editar Impresora: <?php echo htmlspecialchars($nombre_val); ?></h1>
...
<div class="form-container">
    <form action="edit_impresora.php?id=<?php echo htmlspecialchars($impresora_id); ?>" method="POST" class="data-form">
        <div class="form-grid">
        ...
        <div class="form-actions">
            <button type="submit" class="button primary">Actualizar Impresora</button>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function toggleIpAndUrlFields() {
        ...
        if (selectedValue === 'Red' || selectedValue === 'Wi-Fi') {
            ...
            // IMPORTANTE: NO limpiar valores aquí en 'edit', ya que podríamos estar cargando datos existentes.
            // La limpieza se maneja en el backend si el tipo de conexión cambia al enviar el formulario.
        }
    }
